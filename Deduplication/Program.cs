    ...
    class Program
    {
        ...
        static Dictionary<String, Double> ComputeIsCoreProbabilityNameModel(string placeName)
        {
            if (AddOrUpdatePlaceName(placeName))
                ExpectationMaximization(1, 1E-3);
            return isCoreWordProbability[placeName];
        }

        static double ComputeIsCoreProbabilitySpatialContextModel(Dictionary<String, Double> probabilities)
        {
        ...
        static double ComputeIsCoreProbabilityNameModel(Dictionary<String, Double> probabilities)
        {
            ...
            else
            {
                ...
                {
                    double backgroundProbability = backgroundWordDistribution[word];
                ...
                if (nullBackgroundProbability)
                {
                    ...
                    foreach (String word in keys)
                    {
                        ...
                        foreach (String otherWord in keys)
                        {
                            if (otherWord == word)
                                continue;
                            product *= backgroundWordDistribution[otherWord];
                        }
                        denominator += product;
                    }
                }
                ...
                foreach (String word in keys)
                {
                    ...
                    if (nullBackgroundProbability)
                    {
                        ...
                        foreach (String otherWord in keys)
                        {
                            if (otherWord == word)
                                continue;
                            numerator *= backgroundWordDistribution[otherWord];
                        }
                    }
                    else
                        numerator /= backgroundWordDistribution[word];
        ...
        {  
            if (model == Model.Name)
            {
                ...
                // Update backgroundWordDistribution
                keys = new List<String>(backgroundWordDistribution.Keys);
        ...
        static void ExpectationMaximization(int maxNumSteps, double threshold)
        {
            for (int step = 0; step < maxNumSteps; ++step)
            {
                double variation = ExpectationStep();
        ...
        static bool AddOrUpdatePlaceName(Place place, int tileId = PlacesDatabase.UndefinedTileId)
        {
            return AddOrUpdatePlaceName(place.title, tileId);
        }
        ...
        static void Main(string[] args)
        {
            ...
            foreach (Place place in database.Places)
                AddOrUpdatePlaceName(place, database.GetTileId(place));
            ...
            ExpectationMaximization(1000, 1E-3);
